  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `<h4>Hello, ${userName}</h4>
                   ...
                   <a href="http://147.93.97.199:3000/api/auth/verify/${verificationToken}">Verify Email</a>`,
    };
  ...
  try {
    ...
    if (patient.verified !== "yes") {
      return res.status(401).json({
        ...
        body: null,
        message:
          "Your email address has not been verified. Please check your email to verify your account.",
      });
    }
...
const sendPasswordResetEmail = async (req, res) => {
  ...
  try {
    ...
    const resetUrl = `http://147.93.97.199:3000/api/auth/reset/${resetToken}?userType=${userType}`;
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: email,
