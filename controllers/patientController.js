...
const getNearestDoctors = async (req, res) => {
  ...
  try {
    console.log(
      "Finding nearest doctors for coordinates:",
      latitude,
      longitude,
    );
    ...
    subscriptions.forEach((sub) => {
      if (sub.clinisist) {
        // Check if clinisist is not null
        subscribedClinisistMap.set(sub.clinisist.toString(), sub.plan);
      } else {
        console.warn(
          `Subscription with ID ${sub._id} has no associated clinisist.`,
        );
      }
    });
    ...
    const nearestClinisists = clinisists
      ...
      .map((clinisist) => {
        ...
        const isSubscribed = subscribedClinisistMap.has(
          clinisist._id.toString(),
        )
          ? 1
          : 0;
        const planDetails =
          subscribedClinisistMap.get(clinisist._id.toString()) || null;
...
const getAllOrganizations = async (req, res) => {
  try {
    const organizations = await Organization.find({ active: true }).select(
...
const getOrganizationDoctors = async (req, res) => {
  try {
    ...
    // Log for debugging
    console.log(
      `Found ${doctors.length} doctors for organization ${organizationId}`,
    );
