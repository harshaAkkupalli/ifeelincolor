...
const getClinisistProfile = async (req, res) => {
  try {
    ...
    const clinisistData = req.clinisist.toObject
      ? req.clinisist.toObject()
      : req.clinisist;
...
const updateDoctor = async (req, res) => {
  try {
    ...
    clinisist.licenseExpirationDate =
  ...
  try {
    ...
    const subscribedPatients = patients.map((patient) => {
      const subscription = subscriptions.find((sub) =>
        sub.patient.equals(patient._id),
      );
      return {
        patient: patient,
        subscription: {
...
const getSubscribedPatientsAssessments = async (req, res) => {
  try {
    ...
    const subscribedPatientsWithAssessments = patientsWithAssessments.map(
      (patient) => {
        const subscription = subscriptions.find((sub) =>
          sub.patient.equals(patient._id),
        );
    ...
    res.status(500).json({
      ...
      body: null,
      message:
        "An error occurred while fetching subscribed patients' assessments",
    });
  }
};
...
const getClinicistSubscriptionStats = async (req, res) => {
  try {
    ...
    const percentageChange =
      totalSubscribedPatients > 0
        ? (
            (lastMonthSubscribedPatients / totalSubscribedPatients) *
            100
          ).toFixed(2)
        : 0;
...
const getClinicistSalesStats = async (req, res) => {
  try {
    ...
    const lastMonthSalesValue =
      lastMonthSales.length > 0 ? lastMonthSales[0].lastMonthSales : 0;
...
const getClinicistRecommendationStats = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      body: null,
      message:
        "An error occurred while fetching clinicist recommendation stats",
    });
  }
};
...
const getClinicistRecommendationsAndPatients = async (req, res) => {
  try {
    ...
    const patientRecommendations = recommendations.reduce((acc, rec) => {
      const patientId = rec.recommendedTo
        ? rec.recommendedTo._id.toString()
        : "unknown";
      if (!acc[patientId]) {
        acc[patientId] = {
          patient: rec.recommendedTo
            ? rec.recommendedTo
            : { _id: "unknown", userName: "Patient not found" },
          recommendations: [],
        };
      }
    ...
    const formattedResponse = Object.values(patientRecommendations).map(
    ...
    console.error(
      "Error fetching clinicist recommendations and patients:",
      error,
    );
    res.status(500).json({
      status: "error",
      body: null,
      message:
        "An error occurred while fetching clinicist recommendations and patients",
    });
  }
};
...
const getClinisistById = async (req, res) => {
  try {
    const clinisist = await Clinisist.findById(req.params.id).select(
      "-password -verificationToken -tokenExpiration -resetPasswordToken -resetPasswordExpires",
    );
