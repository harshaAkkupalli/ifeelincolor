        ...
        for (const mediaType of ["images", "documents", "videos"]) {
          if (req.files[mediaType]) {
            for (const file of req.files[mediaType]) {
              ...
              const url = await s3Util.uploadFile(
                file.buffer,
                key,
                file.mimetype,
              );
    ...
    const deleteMedia = async (mediaArray) => {
      for (let media of mediaArray) {
        try {
          ...
          console.error(
            `Error deleting file with public_id ${media.public_id}:`,
            error,
          );
        }
      }
  ...
  try {
    ...
    if (doctorRecommendations.length === 0) {
      return res.json({
        ...
        body: [],
        message:
          "No doctor recommendations found for this category and patient",
      });
    }
...
const createDoctorRecommendation = (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      console.log("========== CREATE DOCTOR RECOMMENDATION ==========");

      console.log("REQ BODY:", req.body);
      console.log("REQ FILES:", req.files);
      console.log("REQ CLINISIST:", req.clinisist);
      ...
      if (!req.clinisist) {
        return res.status(401).json({
          status: "error",
          body: null,
          message: "Clinisist not authenticated",
        });
      }
      ...
      if (req.files && Object.keys(req.files).length > 0) {
        ...
        for (const mediaType of ["images", "documents", "videos"]) {
          ...
          if (req.files[mediaType] && Array.isArray(req.files[mediaType])) {
            console.log(`${mediaType} COUNT:`, req.files[mediaType].length);
            ...
            console.log(`NO FILES FOUND FOR: ${mediaType}`);
          }
        }
      } else {
      ...
      console.log(
        "FINAL RELATED MEDIA:",
        JSON.stringify(newRecommendation.relatedMedia, null, 2),
      );
      ...
      console.error("CREATE DOCTOR RECOMMENDATION ERROR:", error);
