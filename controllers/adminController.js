...
const verifyDoctor = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error verifying clinician", error: error.message });
  }
};
...
const getPortalPlans = async (req, res) => {
  try {
    const portalPlans = await Plan.find({ planType: "portal-plan" }).sort({
      createdAt: -1,
...
const getPortalPlanById = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      body: null,
      message:
        "An error occurred while fetching the portal plan: " + error.message,
    });
  }
};
...
const updatePortalPlan = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      body: null,
      message:
        "An error occurred while updating the portal plan: " + error.message,
    });
  }
};
...
const getPortalPlanSubscriptions = async (req, res) => {
  try {
    ...
    const portalPlanIds = await Plan.find({ planType: "portal-plan" }).distinct(
      "_id",
    );
...
const getPortalPlanSubscriptionCounts = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      error: error.message,
      message:
        "An error occurred while fetching portal plan subscription counts",
    });
  }
};
...
const deletePortalClinician = async (req, res) => {
  try {
    const clinicianId = req.params.id;
    const clinician = await Clinisist.findOneAndDelete({
      _id: clinicianId,
      organization: null,
    });
...
const getPortalClinicianCounts = async (req, res) => {
  try {
    ...
    const activeCount = await Clinisist.countDocuments({
      organization: null,
      Active: "yes",
    });
    const inactiveCount = await Clinisist.countDocuments({
      organization: null,
      Active: "no",
    });
...
const getPortalPlanPatientAssessments = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      body: null,
      message:
        "An error occurred while fetching portal plan patient assessments",
      error: error.message,
    });
  }
};
...
const getDoctorPlanSubscriptions = async (req, res) => {
  try {
    ...
    const doctorPlanIds = await Plan.find({ planType: "doctor-plan" }).distinct(
      "_id",
    );
...
const getDoctorPlanSubscriptionsMonthWise = async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      error: error.message,
      message:
        "An error occurred while fetching doctor plan subscription counts",
    });
  }
};
...
const getDoctorPlanSubscriptionsWithDetails = async (req, res) => {
  try {
    ...
    const subscriptions = await Subscription.find({
      ...dateFilter,
      plan: {
        $in: await Plan.find({ planType: "doctor-plan" }).distinct("_id"),
      },
    })
      .populate("plan")
      .populate("clinisist")
    ...
    res.status(200).json({
      ...
      count: formattedSubscriptions.length,
      message:
        "Doctor plan subscriptions with full details retrieved successfully",
    });
  } catch (error) {
    console.error(
      "Error fetching doctor plan subscriptions with details:",
      error,
    );
    res.status(500).json({
      status: "error",
      error: error.message,
      message:
        "An error occurred while fetching doctor plan subscriptions with details",
    });
  }
};
...
const calculateTotalEarnings = async (req, res) => {
  try {
    ...
    const patientTotal =
      patientEarnings.length > 0 ? patientEarnings[0].totalEarnings : 0;
    const clinicianTotal =
      clinicianEarnings.length > 0 ? clinicianEarnings[0].totalEarnings : 0;
    const organizationTotal =
      organizationEarnings.length > 0
        ? organizationEarnings[0].totalEarnings
        : 0;
    ...
    res.status(200).json({
      ...
      message:
        "Total earnings calculated successfully for patient, clinician, and organization subscriptions",
    });
  } catch (error) {
...
const getSubscriptionCountsMonthWise = async (req, res) => {
  try {
    ...
    const organizationSubscriptionCounts =
      await OrganizationSubscription.aggregate([
        {
          $match: {
            createdAt: { $gte: start, $lte: end },
          },
...
const getTotalSubscriptionCounts = async (req, res) => {
  try {
    ...
    const results = {
      patientSubscription: patientSubscriptionCounts[0] || {
        active: 0,
        renewal: 0,
        ended: 0,
      },
      clinicianSubscription: clinicianSubscriptionCounts[0] || {
        active: 0,
        renewal: 0,
        ended: 0,
      },
      organizationSubscription: organizationSubscriptionCounts[0] || {
        active: 0,
        renewal: 0,
        ended: 0,
      },
  ...
  try {
    ...
    if (socialMediaLinks) {
      admin.socialMediaLinks = {
        ...
        instagram:
          socialMediaLinks.instagram || admin.socialMediaLinks.instagram,
...
const updateAdminMedia = async (req, res) => {
  try {
    ...
    if (req.files && req.files.image) {
      ...
      // Upload new profile image
      const fileKey = `admin_images/${Date.now()}_${profileFile.name}`;
    ...
    // ================= COMPANY IMAGE =================
    if (req.files && req.files.companyImage) {
      const companyFile = Array.isArray(req.files.companyImage)
        ? req.files.companyImage[0]
      ...
      // Upload new company image
      const fileKey = `company_images/${Date.now()}_${companyFile.name}`;
...
const getNonVerifiedClinicians = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      message:
        nonVerifiedClinicians.length > 0
