...
const getAssistantProfile = async (req, res) => {
  try {
    const assistantData = req.assistant.toObject
      ? req.assistant.toObject()
      : req.assistant;
...
const updateAssistantInfo = async (req, res) => {
  try {
    ...
    const { name, email, address, socialMediaLinks, contact, bio } = req.body;
    ...
    const updatedPermissions = await AssistantPermission.findOneAndUpdate(
      { assistantId },
      {
        permissions: {
          ...
          organizationSubscription: Boolean(
            permissions.organizationSubscription,
          ),
