...
const createBodyAssessment = async (req, res) => {
    try {
        const { question, answer, type, score, mcqOptions, part, mainColor, mainFeeling } = req.body;
        ...
        if (!type || !['mcq', 'blanks'].includes(type)) {
            return res.status(400).json({
                status: 'error',
                body: null,
                message: 'Type must be either "mcq" or "blanks"'
            });
        }
        ...
        // Validate part is a valid ObjectId
        const mongoose = require('mongoose');
        if (!mongoose.Types.ObjectId.isValid(part)) {
            return res.status(400).json({
                status: 'error',
                body: null,
                message: 'Invalid body part ID format'
            });
        }

        if (type === 'mcq' && (!mcqOptions || !Array.isArray(mcqOptions) || mcqOptions.length === 0)) {
            return res.status(400).json({
                status: 'error',
                body: null,
                message: 'MCQ options are required for MCQ type assessments'
            });
        }

        // Ensure mcqOptions have the required structure with emotion hierarchy
        const formattedMcqOptions = (mcqOptions || []).map(option => ({
            text: option.text || '',
            color: option.color || null,
            narration: option.narration || '',
            emotionHierarchy: option.emotionHierarchy || null,
        ...
        // Generic error message with more details in development
        const errorMessage = process.env.NODE_ENV === 'development' 
            ? `Error creating body assessment: ${error.message}`
            : 'An error occurred while creating the body assessment. Please check all required fields are provided correctly.';
...
const updateBodyAssessment = async (req, res) => {
    try {
        ...
        // Format mcqOptions with all fields including narration and emotionHierarchy
        ...
        const updatedBodyAssessment = await BodyAssessment.findByIdAndUpdate(req.params.id, updatedData, { new: true, runValidators: true });
